...
export default {
  ...
  methods: {
    updateStore() {
      this.$emit('value', { id: this.Id, answer: this.arrayOfImageObjects });
    },
    async previewImage(e) {
      const files = e.target.files;

      const $thisImagePreview = e.target
        .closest('.task__camera')
        .querySelector('img.output');
