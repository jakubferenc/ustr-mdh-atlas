...
export default {
  ...
  computed: {
    moznostiPrepared() {
      if (
        this.MoznostiContainer.autoGenerateId &&
        this.MoznostiContainer.autoGenerateId == 'true'
      ) {
        if (!this.MoznostiContainer.slug) {
          throw new Exception(
            'For autogenerating IDs, you need to pass the "Slug" variable into the component'
          );
        }
  ...
  methods: {
    updateStore() {
      this.$emit('value', { id: this.Id, answer: this.selectedItems });
    },
